...
function createSections() {
	...
	let d = Math.floor(width / numPoints);
...
function genEdge(x1, y1, x2, y2, tile_name = 0) {
	...
	let localWidth = find_width(point.x, point.y, x1, y1, x2, y2);
	...
	while (distance > 0 && !exit) {
		...
		distance = find_distance(point.x, point.y, x2, y2, initial_angle);
		localWidth = find_width(point.x, point.y, x1, y1, x2, y2);
		//delete
		//exit = true;
	}

	localWidth = find_distance(point.x, point.y, x2, y2)
	angle = find_center_angle(point.x, point.y, x2, y2)
	while (localWidth > 0 && !exit) {
		point = map_section_edge(point.x, point.y, angle, tile_name)
		localWidth = find_distance(point.x, point.y, x2, y2)
		//delete
		exit = true;
...
function find_width(x, y, x1, y1, x2, y2) {
	...
	if (x2 - x1 != 0) {
		let m = (y2 - y1) / (x2 - x1);
		let b = y2 - m * x2;
		localWidth = Math.abs(y - (m * x) - b) / (Math.sqrt(m ** 2 + 1));
	}
	else {
		localWidth = Math.abs(x2 - x);
	}
	return localWidth;
